...
> **📊 Why Metrics May Differ from vLLM**: LlamaStack metrics track your application's token usage, while vLLM metrics show the shared infrastructure view. Since multiple users share the vLLM model in `ai501`, the dashboards show different perspectives - your app's consumption vs. total infrastructure load.
