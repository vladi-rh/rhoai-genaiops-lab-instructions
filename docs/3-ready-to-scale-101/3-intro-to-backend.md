...
1. Now it is time to make Canopy UI to talk with backend, instead of directly sending requests to the LLM. In order to do that, we need to update some values in our helm chart. In the `Workloads` >  `Topology` view, find `frontend` and click the three dots underneath > `Upgrade`
